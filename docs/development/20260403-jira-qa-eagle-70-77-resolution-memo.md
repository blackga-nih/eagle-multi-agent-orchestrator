...
| EAGLE-71 | Missing NIH-specific policy layer in threshold answers | No NIH policy overlay prompt; research tool skipped 79% of cascades | Composite research tool with auto-fetch; PMR/FRC checklist enforcement | `7d3e8ac`, `98bc7ab`, `a522b2d` |
...
| EAGLE-74 | Wrong FAR section (16.507 vs 16.505), missing 7th exception | KB uses HHS RFO numbering; agent cited RFO sections to users | RFO-to-FAR cross-reference in system prompt | `1bb4c46` |
...
| EAGLE-77 | Template path not shown before document generation | Fast-path bypassed agent; no template lookup | Fast-path template search + agent-level guardrail | `1bb4c46` |
...
**Validation:** Response (6,196 chars) correctly retrieves and cites B-302358 via `legal-counselor/appropriations-law/GAO_B-302358_IDIQ_Min_Fund.txt`, uses `knowledge_search` -> `search_far` -> `knowledge_fetch` -> `web_search` -> `web_fetch` cascade.
...
**Fix (commit `1bb4c46`):**
1. **System prompt update**: Added FAR SECTION ACCURACY guidance with explicit RFO-to-FAR mappings (16.507-6 = 16.505(b)(2)(i), etc.) and instruction to never cite 16.507-x to users. Includes note that there are seven (7) exceptions and (F) small business set-asides appears as paragraph (c) in the RFO document.

**Validation:** Re-run produces all 7 exceptions (A-G) under "The Seven Exceptions — FAR 16.505(b)(2)(i)", cites 16.505 correctly, includes (F) Small Business Set-Asides, zero 16.507 references.
...
**Validation:** Response (10,596 chars) uses `knowledge_search` + `knowledge_fetch`, fetches 9 protest guidance documents including `GAO_Bid_Protests_Essential_Guide_for_COs.txt`, `NIH_6033_2_Stay_Provisions_Protests.txt`, and `GAO_B-419956.258_EPDS_Filing_Timeliness.txt`. Covers GAO bid protest procedures and stay provisions.
...
|------|--------|-------------|
| 2026-04-02 | `af83106` | KB cascade enforcement + compliance matrix deep research |
| 2026-04-03 | `98bc7ab` | Enforce checklist lookup before document recommendations |
| 2026-04-03 | `8160721` | Auto-fetch PMR checklist content in query_compliance_matrix response |
| 2026-04-03 | `7d3e8ac` | Add composite research tool — dynamic KB + checklist in one call |
| 2026-04-03 | `c2a95e8` | Fix KB search overflow, safeguard wipe script, add local CloudWatch logging |
| 2026-04-03 | `a522b2d` | Isolate checklist fetching in research tool |
| 2026-04-03 | `1bb4c46` | Fix EAGLE-74 FAR numbering and EAGLE-77 template visibility |
