"""Generate the v2 PowerPoint deck for the compliance officer's template review.

v2 differences from v1 deck:
- Title slide says v2 + KB-grounded
- New "What changed since v1" slide
- New "Material differences caught" table slide
- Per-template slides parse content from the v2 markdown files (so deck stays
  in sync with whatever the subagents wrote, vs hardcoding it here)
- KB-grounded badge on each template slide (green = KB grounded, amber = model only)
- Approval matrix shows version + KB indicator per row
"""
...
OUT_PPTX = REPO_ROOT / "docs/development/20260505-171152-report-compliance-template-review-v4.pptx"
...
TEMPLATES = [
    ("cor_designation", "v2", "COR Designation Letter", True, "NIH COR Handbook + FAQ"),
...
for i, line in enumerate([
    ...
    "v2 supersedes v1 - six templates redrafted with EAGLE knowledge base sources",
]):
...
# 3. Material differences caught
s = prs.slides.add_slide(prs.slide_layouts[6])
navy_band(s, prs)
slide_title(s, "Material differences v1 missed")

diffs = [
    ("cor_designation", "Six prescribed responsibility categories from NIH COR Handbook Appendix 8A; FAC-COR dollar tiers (I/II/III); separate signature page"),
...
for idx, (slug, version, doc_name, kb_grounded, kb_label) in enumerate(TEMPLATES, 1):
    ...
    # Required Sections — full slide width (worked example removed; full
    # detail lives in the template markdown file)
    ...
    # Footer pointer to the full template (incl. worked example)
