# EAGLE Documentation

## Getting Started

| Guide | Description |
|-------|-------------|
| [Local Development](setup/local-development.md) | Docker Compose on your laptop |
| [EC2 Runner Deployment](setup/ec2-runner-deployment.md) | Standard NCI deploy path |
| [CDK Bootstrap](setup/cdk-bootstrap.md) | First-time AWS account setup |

## Architecture

- [Codebase Structure](codebase-structure.md)
- [Architecture Diagrams](architecture/) — system design, Excalidraw diagrams
- [Tech Stack Analysis](architecture/tech-stack-analysis.md)
- [Asset Ownership](architecture/asset-ownership.md)

## Development

- [Reports & Audits](development/) — sprint reports, code reviews, triage
- [GitHub Workflow](development/github-workflow.md)
- [Langfuse Trace Guide](development/langfuse-trace-aggregation-guide.md)

## Plans

- [Implementation Plans](plans/) — feature specs, migration runbooks

## Deployment

- [Production Validation Checklist](deployment/EAGLE-production-validation-checklist.md)

## Demo

- [Demo Script](demo/EAGLE-DEMO-SCRIPT.md)
