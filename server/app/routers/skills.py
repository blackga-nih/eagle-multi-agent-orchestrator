...
Provides endpoints for user-created skills management:
- List/create/get/update skills
- Submit for review, publish, delete
"""
...
async def submit_skill_endpoint(
    ...
    if not skill:
        raise HTTPException(
            status_code=404,
            detail="Skill not found or not in draft status",
        )
    return skill
...
async def publish_skill_endpoint(
    ...
    if not skill:
        raise HTTPException(
            status_code=404,
            detail="Skill not found or not in review status",
        )
    write_audit(
        tenant_id=user.tenant_id,
        entity_type="skill",
...
async def delete_skill_endpoint(
    ...
    if not ok:
        raise HTTPException(
            status_code=400,
            detail="Skill not found or cannot be deleted (must be draft or disabled)",
        )
