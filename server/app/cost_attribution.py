...
class CostAttributionService:
    ...
    async def calculate_tenant_costs(
        ...
        for metric in usage_metrics:
            ...
            if metric_type == "bedrock_input_tokens":
                ...
                token_cost = (value / 1000) * self.bedrock_pricing[model_id][
                    "input_tokens"
                ]
    ...
    async def calculate_user_costs(
        ...
        tenant_costs = await self.calculate_tenant_costs(
            tenant_id, start_date, end_date
        )
        user_costs = tenant_costs["users"].get(
            user_id,
            {
                "bedrock_cost": 0.0,
    ...
    async def track_usage_cost(
        ...
        **metadata,
        ...
        if metric_type == "bedrock_input_tokens":
            ...
            cost = (Decimal(str(value)) / 1000) * self.bedrock_pricing[model_id][
                "input_tokens"
            ]
        ...
        if cost > 0:
            session_store.store_cost_metric(
                ...
                f"{metric_type}_cost",
                float(cost),
                original_metric=metric_type,
                **metadata,
            )

    async def get_subscription_tier_costs(
        self, tier: SubscriptionTier, days: int = 30
    ) -> Dict[str, Any]:
        ...
        for tenant in tenants_by_tier:
            ...
            tier_breakdown["weather_api_costs"] += Decimal(
                str(breakdown["weather_api_costs"])
            )
            tier_breakdown["agent_runtime_costs"] += Decimal(
                str(breakdown["agent_runtime_costs"])
            )
            tier_breakdown["total_invocations"] += breakdown["total_invocations"]
            tier_breakdown["total_tokens"]["input"] += breakdown["total_tokens"][
                "input"
            ]
            tier_breakdown["total_tokens"]["output"] += breakdown["total_tokens"][
                "output"
            ]

        avg_cost_per_tenant = (
            total_cost / tenant_count if tenant_count > 0 else Decimal("0")
        )
