...
class CognitoAuth:
    ...
    def verify_token(self, token: str) -> Dict:
        ...
        try:
            ...
            role = (
                "admin"
                if any(group.endswith("-admins") for group in user_groups)
                else "user"
            )

            if not tenant_id:
                raise HTTPException(
                    status_code=403, detail="No tenant ID found in token"
                )
...
async def get_current_user(
    ...
    if DEV_MODE and (
        credentials is None or credentials.credentials == "dev-mode-token"
    ):
        return {
