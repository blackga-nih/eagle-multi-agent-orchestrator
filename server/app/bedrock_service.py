...
class BedrockAgentService:
        ...
        try:
            if self.agent_id and self.agent_id != "your-agent-id":
                ...
                return self._process_agent_response(
                    response, session_id, tenant_context.tenant_id
                )
            else:
                return self._invoke_claude_direct(
                    message, session_id, tenant_context.tenant_id
                )
    ...
    def _process_agent_response(
        self, response, session_id: str, tenant_id: str
    ) -> Dict[str, Any]:
                ...
                if "orchestrationTrace" in trace:
                    ...
                    if "modelInvocationOutput" in orchestration:
                        ...
                        if "metadata" in model_output:
                            ...
                            usage_data["input_tokens"] = metadata.get("usage", {}).get(
                                "inputTokens", 0
                            )
                            usage_data["output_tokens"] = metadata.get("usage", {}).get(
                                "outputTokens", 0
                            )
    ...
    def _invoke_claude_direct(
        self, message: str, session_id: str, tenant_id: str
    ) -> Dict[str, Any]:
