...
class AdminCostService:
    ...
    def __init__(self):
        ...
        self.service_pricing = {
            ...
            "weather_api": {"api_call": Decimal("0.0001")},
            "mcp_runtime": {"tool_execution": Decimal("0.0005")},
            "session_management": {
    ...
    async def get_tenant_overall_cost(
    ...
    async def get_tenant_per_user_cost(
        self, tenant_id: str, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        ...
        for metric in usage_metrics:
            ...
            user_costs[user_id]["service_costs"][service_cost["service"]] += (
                service_cost["amount"]
            )
            ...
            if metric["metric_type"] == "agent_invocation":
                ...
                user_costs[user_id]["usage_stats"]["tokens"]["input"] += int(
                    metric["value"]
                )
            elif metric["metric_type"] == "bedrock_output_tokens":
                user_costs[user_id]["usage_stats"]["tokens"]["output"] += int(
                    metric["value"]
                )
        ...
        return {
            ...
            "users": {
                user_id: {
                    ...
                    "service_costs": {
                        k: float(v) for k, v in user_data["service_costs"].items()
                    },
    ...
    async def get_tenant_service_wise_cost(
        self, tenant_id: str, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        ...
        for metric in usage_metrics:
            ...
            if (
                service_breakdown[service]["daily_costs"][date_key]
                > service_breakdown[service]["peak_usage_day"]["cost"]
            ):
            ...
            "service_breakdown": {
                service: {
                    ...
                    "average_cost_per_use": float(
                        data["total_cost"] / data["usage_count"]
                    )
                    ...
                    "daily_trend": {
                        k: float(v) for k, v in data["daily_costs"].items()
                    },
                }
    ...
    async def get_user_service_wise_cost(
        self, tenant_id: str, user_id: str, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        ...
        for metric in user_metrics:
            ...
            if session_id not in user_service_breakdown[service]["session_costs"]:
                user_service_breakdown[service]["session_costs"][session_id] = Decimal(
                    "0"
                )
        ...
        return {
            ...
            "user_service_breakdown": {
                service: {
                    ...
                    "usage_count": data["usage_count"],
                    "cost_per_hour": {
                        k: float(v) for k, v in data["hourly_usage"].items()
                    },
                    "cost_per_session": {
                        k: float(v) for k, v in data["session_costs"].items()
                    },
                    "average_cost_per_use": float(
                        data["total_cost"] / data["usage_count"]
                    )
    ...
    def _calculate_service_cost(self, metric: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if metric_type == "bedrock_input_tokens":
            return {
                ...
                "amount": (value / 1000)
                * self.service_pricing["bedrock_agent"]["input_tokens"],
            }
        elif metric_type == "bedrock_output_tokens":
            return {
                "service": "bedrock_agent",
                "amount": (value / 1000)
                * self.service_pricing["bedrock_agent"]["output_tokens"],
            }
            ...
            return {
                ...
                "amount": self.service_pricing["session_management"][
                    "session_creation"
                ],
            }
    ...
    async def generate_comprehensive_admin_report(
        ...
        overall_cost = await self.get_tenant_overall_cost(
            tenant_id, start_date, end_date
        )
        per_user_cost = await self.get_tenant_per_user_cost(
            tenant_id, start_date, end_date
        )
        service_wise_cost = await self.get_tenant_service_wise_cost(
            tenant_id, start_date, end_date
        )
        ...
        return {
            ...
            "summary": {
                ...
                "highest_cost_service": max(
                    overall_cost["service_breakdown"].items(), key=lambda x: x[1]
                )[0],
            ...
            "4_top_users_by_cost": {
                user_id: user_data for user_id, user_data in top_users
            },
        }
