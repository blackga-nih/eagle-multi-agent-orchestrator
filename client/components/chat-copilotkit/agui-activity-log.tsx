...
function DetailModal({
  ...
  useEffect(() => {
    const handle = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setActiveIndex((i) => Math.min(allDisplay.length - 1, i + 1));
      }
    };
  ...
  const handleCopy = async () => {
    const payload =
      active.entries.length > 1 ? active.entries.map((e) => e.event) : active.entries[0].event;
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div
          className={`px-4 py-3 border-b flex items-center justify-between shrink-0 ${active.bg}/10`}
        >
          <div className="flex items-center gap-2">
            <span
              className={`w-6 h-6 rounded-full flex items-center justify-center text-white ${active.bg}`}
            >
              <active.icon className="w-3 h-3" />
            </span>
            <span className="text-sm font-bold text-gray-800">{active.label}</span>
            <span className="text-xs text-gray-400 font-mono">
              {active.entries[0].receivedAt
                ? new Date(active.entries[0].receivedAt).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit',
                    second: '2-digit',
                  })
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] text-gray-500 font-mono min-w-[3rem] text-center">
              {activeIndex + 1}/{allDisplay.length}
            </span>
            <button
              onClick={() => setActiveIndex((i) => Math.min(allDisplay.length - 1, i + 1))}
              disabled={activeIndex === allDisplay.length - 1}
              className="p-1 hover:bg-black/10 rounded disabled:opacity-30"
            >
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-2 border-b shrink-0">
          <button
            onClick={handleCopy}
            className="flex items-center gap-1 px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 rounded-lg"
          >
          ...
          {showRaw ? (
            <pre className="bg-gray-900 text-green-400 p-4 rounded-xl text-xs overflow-x-auto font-mono whitespace-pre-wrap">
              {JSON.stringify(
                active.entries.length > 1
                  ? active.entries.map((e) => e.event)
                  : active.entries[0].event,
                null,
                2,
              )}
            </pre>
            ...
            <div className="space-y-3">
              ...
              <div className="pt-3 border-t border-gray-100 text-[10px] text-gray-400 font-mono">
                ...
                {active.entries[0].event.run_id &&
                  ` · run:${active.entries[0].event.run_id.slice(0, 8)}`}
                {active.entries[0].event.tool_call_id &&
                  ` · tc:${active.entries[0].event.tool_call_id.slice(0, 8)}`}
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#D8DEE6] shrink-0 flex-wrap">
        ...
        {counts.step > 0 && (
          <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-indigo-100 text-indigo-700">
            {counts.step} subagent{counts.step !== 1 ? 's' : ''}
          </span>
        )}
        {counts.tool > 0 && (
          <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-yellow-100 text-yellow-800">
            {counts.tool} tool{counts.tool !== 1 ? 's' : ''}
          </span>
        )}
        {counts.error > 0 && (
