...
export default function StateChangeCard({ entry }: StateChangeCardProps) {
    ...
    const hasProgress = entry.stateType === 'checklist_update' && (entry.checklist?.required?.length ?? 0) > 0;
    ...
    return (
        <>
            ...
            {/* Detail modal */}
            {showDetail && (
                <Modal isOpen={showDetail} onClose={() => setShowDetail(false)} title="Package State Update">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                            ...
                            {entry.acquisitionMethod && (
                                <>
                                    ...
                                    <span className="text-gray-700 capitalize">{entry.acquisitionMethod.replace(/_/g, ' ')}</span>
                                </>
                            )}
                            {entry.contractType && (
                                <>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-1">
                                    ...
                                    <span className="text-xs font-medium text-[#003366]">
                                        {entry.checklist?.completed?.length ?? 0}/{entry.checklist?.required?.length ?? 0}
                                    </span>
                                </div>
