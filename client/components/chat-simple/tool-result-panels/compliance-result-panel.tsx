...
export default function ComplianceResultPanel({ text }: { text: string }) {
  ...
  return (
    <div className="border-t border-[#E5E9F0] bg-white px-3 py-2.5">
      <div className="text-[9px] font-bold uppercase text-blue-600 tracking-wider mb-1.5">Compliance Check</div>
      ...
      {(data.vehicle || data.recommendation) && (
        <div className="mb-2">
          ...
          {data.reasoning && (
            <p className="text-[10px] text-gray-500 mt-0.5">{data.reasoning}</p>
          )}
        </div>
      )}
