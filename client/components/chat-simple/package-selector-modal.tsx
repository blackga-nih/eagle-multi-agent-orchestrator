  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        {/* Package selection */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Assign to Package</label>
          ...
          {isLoading ? (
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              ...
              >
                ...
                />
                <Folder
                  className={`w-5 h-5 ${selectedPackageId === null ? 'text-blue-500' : 'text-gray-400'}`}
                />
                <div className="flex-1">
                  <p
                    className={`font-medium ${selectedPackageId === null ? 'text-blue-700' : 'text-gray-700'}`}
                  >
                    Save Without Package Assignment
                  </p>
                  <p className="text-xs text-gray-500">
                    Keep this document in your personal uploads
                  </p>
                </div>
              </label>
              ...
              {/* Package options */}
              {packages
                .filter((pkg) => !pkg.status || !['closed', 'archived'].includes(pkg.status))
                .map((pkg) => (
                  <label
                    key={pkg.package_id}
                    className={`
                  ...
                  >
                    ...
                    />
                    <Package
                      className={`w-5 h-5 ${selectedPackageId === pkg.package_id ? 'text-blue-500' : 'text-gray-400'}`}
                    />
                    <div className="flex-1 min-w-0">
                      <p
                        className={`font-medium truncate ${selectedPackageId === pkg.package_id ? 'text-blue-700' : 'text-gray-700'}`}
                      >
                        {pkg.package_id}
                      </p>
