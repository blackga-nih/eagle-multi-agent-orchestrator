...
export default function DocumentChecklist({ messages, data, sessionId }: DocumentChecklistProps) {
  ...
  const getRelevantDataForDoc = (docId: string): boolean => {
    switch (docId) {
      case 'sow':
        return !!(data.requirement || data.equipmentType);
      case 'igce':
        return !!data.estimatedValue;
      case 'market-research':
        return !!(data.requirement || data.equipmentType);
      case 'plan':
        return !!(data.requirement && data.estimatedValue);
      case 'funding':
        return !!data.funding;
      case 'urgency':
        return !!data.urgency;
  ...
  const handleDownload = async (format: 'pdf' | 'docx') => {
    ...
    try {
      ...
      const allDocContent = documents.map((doc) => getDocContent(doc.id)).join('\n\n---\n\n');
        ...
        {documents.map((doc, idx) => (
          ...
          >
            <div className="mt-0.5">
              {doc.status === 'completed' && <CheckCircle2 className="w-5 h-5 text-green-600" />}
              {doc.status === 'in-progress' && (
      ...
      {selectedDoc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
          <div
            ...
            className="bg-white rounded-2xl shadow-2xl w-[85vw] h-[85vh] flex overflow-hidden animate-in zoom-in-95 duration-300"
          >
            {/* Preview Area */}
            <div className="flex-1 flex flex-col bg-white">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                <h3 className="font-bold text-gray-900 text-lg flex items-center gap-2">
                  <FileText className="w-5 h-5 text-blue-600" />
                  {selectedDoc.isTemplate ? 'Preview: ' : ''}
                  {selectedDoc.name}
            ...
            <div className="w-80 border-l border-gray-200 flex flex-col bg-gray-50">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs text-blue-800">
                  I can help you adjust the specifications or add more technical details to this{' '}
                  {selectedDoc.name.split('(')[0].trim()}.
                </div>
              </div>
