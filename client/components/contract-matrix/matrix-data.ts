...
export function isTypeDisabled(method: string, typeId: string): boolean {
  ...
  if ((method === 'fss' || method === 'bpa-call' || method === 'bpa-est') &&
...
export function getRequirements(s: MatrixState): Requirements {
  ...
  if (m !== 'micro') {
    docs.push({ name: s.isServices ? 'SOW / PWS' : 'Statement of Need (SON)', required: true,
  ...
  docs.push({ name: 'Subcontracting Plan', required: needsSubK, note: needsSubK ? 'Required for non-SB > $750K (FAR 19.705)' : s.isSB ? 'Exempt \u2014 small business awardee' : 'Below $750K threshold' });
  ...
  compliance.push({ name: 'Severable Services \u22641yr/period', status: s.isServices ? 'req' : 'na', note: 'FAR 37.106(b), 32.703-3(b)' });
  ...
  return { docs, triggered, notTriggered, compliance, competition, pmr, timeMin, timeMax, riskPct, warnings, errors, feeCaps, isCR, isLOE, isFP };
}
...
export const CONTRACT_TYPE_FACTORS: ContractTypeFactor[] = [
  {
    ...
    options: [
      ...
      { id: 'non-commercial',  label: 'Non-commercial / custom', weights: { fp: -2, cr: 5,  loe: 3 } },
    ],
  },
  {
    ...
    options: [
      ...
      { id: 'sole-source', label: 'Sole source / no competition', weights: { fp: -3, cr: 5,  loe: 4 } },
    ],
  },
  {
    id: 'price-analysis',
    ...
    farRef: 'FAR 15.404-1',
    options: [
      { id: 'strong',   label: 'Catalog / market prices available', weights: { fp: 8,  cr: -5, loe: -2 } },
      { id: 'moderate', label: 'Some comparable pricing exists',    weights: { fp: 3,  cr: 1,  loe: 1 } },
  ...
  },
  {
    ...
    options: [
      { id: 'yes',     label: 'Certified cost/pricing data available', weights: { fp: 2,  cr: 6,  loe: 0 } },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'inadequate', label: 'Inadequate / not evaluated',  weights: { fp: 6,  cr: -10, loe: 5 } },
    ],
  },
  {
