...
export default function GridMatrixTab({ state, onSelectCell }: GridMatrixTabProps) {
  ...
  const summary = useMemo(() => {
    ...
    for (const m of METHODS) {
      for (const t of TYPES) {
        ...
        if (data.invalid) { invalidCount++; continue; }
        validCount++;
        if (!simplest || (data.reqDocs ?? 99) < simplest.docs) simplest = { method: m.label, type: t.label, docs: data.reqDocs!, time: `${data.timeMin}-${data.timeMax}wk` };
        if (!heaviest || (data.reqDocs ?? 0) > heaviest.docs) heaviest = { method: m.label, type: t.label, docs: data.reqDocs!, time: `${data.timeMin}-${data.timeMax}wk` };
      }
    }
  ...
  const typeShortLabel: Record<string, string> = {
    'ffp': 'FFP', 'fp-epa': 'FP-EPA', 'fpi': 'FPI', 'cpff': 'CPFF',
    'cpif': 'CPIF', 'cpaf': 'CPAF', 'tm': 'T&M', 'lh': 'LH',
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-6 px-5 py-3 border-b border-gray-100 bg-gray-50/50 flex-shrink-0">
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Dollar Value</span>
          <span className="text-sm font-bold font-mono text-amber-600">{fmtDollar(gridDollar)}</span>
          <input
            type="range"
            min="0" max="22" step="0.01"
          ...
          {([
            ...
            >
              <div className={`w-6 h-3.5 rounded-full relative transition-colors ${
                ...
                <div className={`absolute top-[1px] w-3 h-3 bg-white rounded-full transition-all shadow-sm ${
            ...
            <tr>
              ...
              {TYPES.map(t => (
                <th key={t.id} className="px-2 py-2 text-center">
                  ...
                  <div className="text-[9px] text-gray-400">{t.category === 'fp' ? 'Fixed-Price' : t.category === 'cr' ? 'Cost-Reimb' : 'Level-of-Effort'}</div>
                </th>
              ))}
            </tr>
          </thead>
                ...
                {TYPES.map(t => {
                  ...
                  return (
                    <td key={t.id} className="px-1 py-1">
                      ...
                      >
                        <div className="text-xl font-bold" style={{ color: clr.text }}>{data.reqDocs}</div>
                        <div className="text-[9px] font-medium" style={{ color: clr.text }}>DOCS</div>
                        {/* Risk bar */}
                        <div className="w-10/12 h-1 rounded-full overflow-hidden flex mt-0.5">
                          <div className="bg-green-500 h-full" style={{ width: `${100 - (data.riskPct ?? 50)}%` }} />
                          <div className="bg-red-400 h-full" style={{ width: `${data.riskPct ?? 50}%` }} />
                        </div>
                        {/* Badges */}
                        <div className="flex gap-0.5 mt-0.5">
                          {(data.errors ?? 0) > 0 && <span className="text-[7px] px-1 rounded bg-red-200 text-red-700 font-bold">ERR</span>}
                          {data.hasJA && <span className="text-[7px] px-1 rounded bg-amber-200 text-amber-700 font-bold">J&A</span>}
                          {data.hasDF && <span className="text-[7px] px-1 rounded bg-orange-200 text-orange-700 font-bold">D&F</span>}
                          {(data.warnings ?? 0) > 0 && !(data.errors ?? 0) && <span className="text-[7px] px-1 rounded bg-amber-100 text-amber-600 font-bold">!</span>}
                        </div>
                      </div>
                    </td>
                  );
      ...
      <div className="flex items-center gap-4 px-5 py-2 border-t border-gray-100 bg-gray-50/50 text-[10px] text-gray-500 flex-shrink-0">
        <LegendItem color="rgba(34,197,94,0.12)" border="rgba(34,197,94,0.3)" label="1-3 docs (simple)" />
        <LegendItem color="rgba(234,179,8,0.12)" border="rgba(234,179,8,0.3)" label="4-6 docs (moderate)" />
        <LegendItem color="rgba(249,115,22,0.12)" border="rgba(249,115,22,0.3)" label="7-9 docs (complex)" />
        <LegendItem color="rgba(239,68,68,0.12)" border="rgba(239,68,68,0.3)" label="10+ docs (heavy)" />
        <div className="flex items-center gap-1"><div className="w-3 h-3 rounded bg-gray-50 border border-gray-200 opacity-40" /> N/A</div>
      ...
      <div className="px-5 py-2 border-t border-gray-100 text-[11px] font-mono text-gray-500 flex-shrink-0">
        Contract Requirements Matrix at {fmtDollar(gridDollar)}.
        {' '}{summary.validCount} valid, {summary.invalidCount} prohibited.
        {summary.simplest && <> Simplest: {summary.simplest.method} + {summary.simplest.type} ({summary.simplest.docs} docs, {summary.simplest.time}).</>}
        {summary.heaviest && <> Most complex: {summary.heaviest.method} + {summary.heaviest.type} ({summary.heaviest.docs} docs, {summary.heaviest.time}).</>}
      </div>
...
function CellPopover({
  ...
  return (
    <div
      ...
      style={{ left, top }}
    >
      <div className="font-semibold text-sm text-gray-900">{mObj.label} + {tObj.label}</div>
      <div className="text-[11px] text-gray-400">
        Risk: Gov {100 - (data.riskPct ?? 50)}% / Con {data.riskPct ?? 50}% &mdash; {data.timeMin}\u2013{data.timeMax} wk
      </div>

      {data.docs && (
        <div>
          <div className="text-[10px] font-bold uppercase text-gray-400 mb-1">Required Documents</div>
          {data.docs.filter(d => d.required).map((d, i) => (
