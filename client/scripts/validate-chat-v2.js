...
(async () => {
  ...
  page.on('response', async (res) => {
    if (res.url().includes('copilotkit') && res.url().includes('info')) {
      try {
        ...
        console.log(
          `[NET] Response ${res.status()} from ${res.url()} (could not read body: ${e.message})`,
        );
      }
    }
  });
  ...
  if (hasError) {
    ...
    const errorText =
      bodyText.match(/useAgent.*?\./) || bodyText.match(/Something went wrong.*?\./);
  ...
  await page.screenshot({
    path: 'C:/Users/blackga/Desktop/eagle/sm_eagle/screenshots/validate-chat-v2-final-initial.png',
    fullPage: false,
  });
  ...
  console.log(
    'CopilotKit/AG-UI badge present:',
    bodyText.includes('CopilotKit') || bodyText.includes('AG-UI'),
  );
  console.log(
    'Events panel present:',
    bodyText.includes('AG-UI Events') || bodyText.includes('Events'),
  );
  ...
  if (!bodyText.includes('Something went wrong')) {
    ...
    if (inputEl) {
      ...
      await page.screenshot({
        path: 'C:/Users/blackga/Desktop/eagle/sm_eagle/screenshots/validate-chat-v2-final-events.png',
        fullPage: false,
      });
      ...
      const eventMatches = afterText.match(
        /RUN_STARTED|TEXT_MESSAGE|TOOL_CALL|STATE_SNAPSHOT|RUN_FINISHED|STEP_STARTED|STEP_FINISHED/g,
      );
      if (eventMatches) {
