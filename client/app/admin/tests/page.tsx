...
function triggerBadge(trigger: string) {
  ...
  return (
    <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-gray-50 text-gray-600 border border-gray-200">
      {trigger.toUpperCase()}
    </span>
  );
}
...
export default function TestResults() {
  ...
  const analytics = useMemo(() => {
    ...
    const olderAvg =
      olderHalf.length > 0
        ? olderHalf.reduce((s, r) => s + r.pass_rate, 0) / olderHalf.length
        : recentAvg;
  ...
  const renderAnalytics = () => {
    ...
    const trendColor =
      a.trend > 1 ? 'text-green-600' : a.trend < -1 ? 'text-red-600' : 'text-gray-500';
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            <p className={`text-3xl font-bold mt-1 ${passRateColor(a.avgPassRate)}`}>
              {a.avgPassRate.toFixed(1)}%
            </p>
            <div className={`flex items-center gap-1 mt-2 text-xs ${trendColor}`}>
              <TrendIcon className="w-3.5 h-3.5" />
              <span>
                {a.trend >= 0 ? '+' : ''}
                {a.trend.toFixed(1)}% trend
              </span>
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            <p className="text-xs text-gray-500 mt-2">
              {runs.filter((r) => r.failed === 0 && r.errors === 0).length}/{a.totalRuns} all-pass
            </p>
          </div>
          <div className="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            <p className="text-xs text-gray-500 mt-2">
              {a.totalTests.toLocaleString()} total tests executed
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          <h3 className="text-sm font-medium text-gray-900 mb-4">
            Pass Rate — Last {a.sparkline.length} Runs
          </h3>
            ...
            {a.sparkline.map((run, i) => {
              ...
              return (
                <div
                  ...
                  title={`${formatDate(run.timestamp)}: ${run.pass_rate}%`}
                >
                  <div
                  ...
                  <div key={trigger} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${passRateBg(avgRate)}`}
                          style={{ width: `${avgRate}%` }}
                        />
                      </div>
                    </div>
                    <span className="text-[10px] text-gray-400">
                      {greenCount}/{suiteRuns.length} green
                    </span>
                  </div>
                );
              })}
              ...
              {a.models.map((model) => {
                ...
                return (
                  <div key={model} className="flex items-center gap-3">
                    <span className="text-xs text-gray-700 font-mono w-40 truncate" title={model}>
                      {model}
                    </span>
                    <div className="flex-1">
                      <div className="flex justify-between text-xs mb-1">
      ...
      return (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center">
          ...
          <p className="text-amber-600 text-sm mt-2">
            Run{' '}
            <code className="bg-amber-100 px-1.5 py-0.5 rounded">python -m pytest tests/ -v</code>{' '}
            to generate results
          </p>
        </div>
      );
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-6 text-xs">
                  ...
                  <div className="text-center">
                    <p className={`font-bold ${passRateColor(run.pass_rate)}`}>{run.pass_rate}%</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4">
            ...
            <p className="text-xl font-bold text-gray-900">
              {run ? formatDuration(run.duration_s) : '--'}
            </p>
