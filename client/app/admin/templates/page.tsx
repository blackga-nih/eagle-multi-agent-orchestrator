...
export default function TemplatesPage() {
  ...
  const fetchData = useCallback(
    async (refresh = false) => {
      ...
      try {
        ...
        setPhases(
          s3Response?.phases && typeof s3Response.phases === 'object'
            ? (s3Response.phases as Record<string, string>)
            : {},
        );
        setPhaseCounts(
          s3Response?.phase_counts && typeof s3Response.phase_counts === 'object'
            ? (s3Response.phase_counts as Record<string, number>)
            : {},
        );
  ...
  async function openS3Preview(card: TemplateCard, e: React.MouseEvent) {
    ...
    try {
      ...
      setS3PreviewData({
        type: 'markdown',
        content: `Error loading preview: ${err}`,
        filename: card.name,
      });
    } finally {
      ...
      <div className="flex flex-col h-screen bg-gray-50">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="p-8">
            ...
            {activeTab === 'gap-analysis' && <ComplianceGapPanel token={null} />}
                      ...
                      >
                          ...
                          <div className="flex items-center gap-1.5 mb-3 text-xs text-amber-700">
                            ...
                            {card.farPartsCovered && card.farPartsCovered.length > 0 && (
                              <span className="text-amber-500">
                                (Part {card.farPartsCovered.slice(0, 3).join(', ')}
                                {card.farPartsCovered.length > 3
                                  ? ` +${card.farPartsCovered.length - 3}`
                                  : ''}
                                )
                              </span>
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                          <div className="flex items-center gap-2">
                            ...
                            {card.phase && (
                              <span
                                className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${phaseColors[card.phase] || 'bg-gray-100 text-gray-600'}`}
                              >
                            ...
                            {/* Doc type badge */}
                            <span
                              className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${docTypeColors[card.docType] || 'bg-gray-100 text-gray-600'}`}
                            >
          ...
          {selectedCard && (
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              <div className="flex items-center gap-2 mb-2">
                <span
                  className={`text-xs font-bold uppercase px-2 py-1 rounded-full ${docTypeColors[selectedCard.docType] || 'bg-gray-100 text-gray-600'}`}
                >
                  {docTypeLabels[selectedCard.docType] || selectedCard.docType}
                </span>
                <Badge
                  variant={selectedCard.source === 'bundled' ? 'default' : 'primary'}
                  size="sm"
                >
          ...
          {previewCard && (
            <div className="space-y-4">
              <div className="flex items-center gap-2 mb-4">
                <span
                  className={`text-xs font-bold uppercase px-2 py-1 rounded-full ${docTypeColors[previewCard.docType] || 'bg-gray-100 text-gray-600'}`}
                >
                  {docTypeLabels[previewCard.docType] || previewCard.docType}
                </span>
                <Badge
                  variant={
                    previewCard.source === 'bundled'
                      ? 'default'
              ...
              {previewCard.farPartsCovered && previewCard.farPartsCovered.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {previewCard.farPartsCovered.map((part) => (
                    <span
                      key={part}
                      className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-amber-50 text-amber-600 border border-amber-200"
                    >
                      FAR Part {part}
                    </span>
                  ))}
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={
                  saving ||
                  !selectedPackageId ||
                  (selectedPackageId === '__new__' && !newPackageTitle.trim())
                }
            ...
            <div className="space-y-4">
              <div className="p-4 bg-gray-50 rounded-xl">
                ...
                <div className="flex items-center gap-2 mt-2">
                  {copyTarget.phase && (
                    <span
                      className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${phaseColors[copyTarget.phase] || 'bg-gray-100 text-gray-600'}`}
                    >
                      {phases[copyTarget.phase] || copyTarget.phase}
                    </span>
                  )}
                  <span
                    className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full ${docTypeColors[copyTarget.docType] || 'bg-gray-100 text-gray-600'}`}
                  >
              ...
              <p className="text-xs text-gray-500">
                This will create a copy of the template in your selected package. You can then
                customize the copy within the package.
              </p>
            </div>
          )}
        </Modal>
