...
async function handleRequest(req: NextRequest) {
  ...
  const isInfoRequest = copilotKitPath === "/info" || (copilotKitPath === "/" && isInfoGet) || isMethodInfoPost;
  if (isInfoRequest && response.ok) {
    try {
      const data = await response.json() as {
        agents?: Array<{ name: string; description: string }> | Record<string, { description: string }>;
        sdkVersion?: string;
        version?: string;
        actions?: unknown[];
      };
