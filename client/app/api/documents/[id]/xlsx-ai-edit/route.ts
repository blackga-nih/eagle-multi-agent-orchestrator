...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  let body: { request: string; session_id?: string; package_id?: string; change_source?: string };
  try {
  ...
  if (!body.request || typeof body.request !== 'string') {
    return NextResponse.json({ error: 'Missing required field: request' }, { status: 400 });
  }
  ...
  try {
    const response = await fetch(
      `${FASTAPI_URL}/api/documents/xlsx-ai-edit/${encodeURIComponent(docKey)}`,
      {
        method: 'POST',
        headers,
        body: JSON.stringify({
          ...
          change_source: body.change_source || 'ai_edit',
        }),
      },
    );
    ...
    return NextResponse.json(
      { error: 'Failed to apply XLSX AI edits', details: String(error) },
      { status: 500 },
    );
  }
}
