...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  let body: { preview_blocks: unknown[]; preview_mode: string; change_source?: string };
  try {
  ...
  if (!Array.isArray(body.preview_blocks) || typeof body.preview_mode !== 'string') {
    return NextResponse.json(
      { error: 'Missing required fields: preview_blocks, preview_mode' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to save DOCX preview edits', details: String(error) },
      { status: 500 },
    );
  }
}
