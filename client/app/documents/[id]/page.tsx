...
// ---------------------------------------------------------------------------
// Utility functions for DOCX preview block normalisation / comparison
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Utility functions for XLSX preview sheet normalisation / comparison
// ---------------------------------------------------------------------------
...
function DocumentChangelogPanel({
    ...
    }, [documentKey, documentType, packageId]);
...
export default function DocumentViewerPage({ params }: PageProps) {
    ...
    useEffect(() => {
        ...
        if (!loadedFromCache) {
            ...
            if (stored2?.content) {
                // Cast through unknown: StoredDocument is a narrower type; the object
                // may have been written with full DocumentInfo fields at runtime.
                const stored2Full = stored2 as unknown as DocumentInfo;
                setDocumentTitle(stored2.title);
                setDocumentType(stored2.document_type);
                setPackageId(stored2.package_id);
                setFileType(stored2Full.file_type || '');
                setContentType(stored2Full.content_type || '');
                setIsBinaryDocument(Boolean(stored2Full.is_binary));
                setDownloadUrl(stored2Full.download_url || null);
                setS3Key(stored2.s3_key || stored2Full.document_id || null);
                setCurrentDocumentId(stored2Full.document_id || null);
                setCurrentVersion(typeof stored2.version === 'number' ? stored2.version : null);
                const nextPreviewBlocks = normalizeDocxPreviewBlocks(stored2Full.preview_blocks);
                const nextPreviewSheets = normalizeXlsxPreviewSheets(stored2Full.preview_sheets);
                setDocxPreviewMode(stored2Full.preview_mode ?? null);
                setDocxPreviewBlocks(nextPreviewBlocks);
                setEditDocxPreviewBlocks(cloneDocxPreviewBlocks(nextPreviewBlocks));
                setXlsxPreviewSheets(nextPreviewSheets);
                setEditXlsxPreviewSheets(cloneXlsxPreviewSheets(nextPreviewSheets));
    ...
    // Poll for version changes when viewing a binary package document
    useEffect(() => {
...
${docSnippet}`;
    ...
    // Sync activeXlsxSheetId when sheets change
    useEffect(() => {
    ...
    const handleDownload = async (format: 'docx' | 'pdf' | 'md') => {
        ...
        try {
            const safeName = documentTitle.replace(/[^a-z0-9]/gi, '_');
            ...
            const isDocxBlob = sig.length >= 4 && sig[0] === 0x50 && sig[1] === 0x4b && sig[2] === 0x03 && sig[3] === 0x04;
            const isPdf = sig.length >= 4 && sig[0] === 0x25 && sig[1] === 0x50 && sig[2] === 0x44 && sig[3] === 0x46;
            if ((format === 'docx' && !isDocxBlob) || (format === 'pdf' && !isPdf)) {
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 flex overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto p-8">
                            <div className={`${isBinaryDocument && isDocxDocument && isEditing ? 'max-w-6xl' : 'max-w-3xl'} mx-auto`}>
                                {isBinaryDocument && isDocxDocument ? (
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        {activeXlsxSheet ? (
                                            <div className="space-y-4 rounded-2xl border border-gray-200 bg-white p-5">
                                                ...
                                                {/* Full HTML table with column headers */}
                                    ...
                                    /* Generic binary document info box */
                                    <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6 text-sm text-blue-950">
                                        <div className="flex items-center gap-2 text-base font-semibold">
                                            <FileText className="h-5 w-5" />
                    ...
                    <div className="flex flex-col bg-gray-50" style={{ width: '35%' }}>
                        {/* Tab header — underline style to match project convention */}
                        ...
                        {/* Changelog tab — DocumentChangelogPanel with polling */}
                        {assistantTab === 'changelog' && (
