...
const EDIT_INTENT_RE =
  /\b(edit|update|revise|modify|change|clear|fill|rewrite|amend|replace|adjust|section)\b/i;
...
function sanitizeContextText(text: string): string {
  return text
    ...
    .replace(
      /AWS_(ACCESS_KEY_ID|SECRET_ACCESS_KEY|SESSION_TOKEN)\s*[:=]?\s*[^\s\n]+/gi,
      '[REDACTED]',
    );
}
...
function collectChangedDocxPreviewBlocks(
  originalBlocks: DocxPreviewBlock[],
  editedBlocks: DocxPreviewBlock[],
): DocxPreviewBlock[] {
...
function normalizeXlsxPreviewSheets(sheets: unknown): XlsxPreviewSheet[] {
  ...
  return sheets.flatMap((sheet) => {
    ...
    if (
      ...
      typeof rawSheet.sheet_id !== 'string' ||
      typeof rawSheet.title !== 'string'
    ) {
      return [];
    }
...
function xlsxPreviewSheetsEqual(left: XlsxPreviewSheet[], right: XlsxPreviewSheet[]): boolean {
  ...
  return left.every((sheet, sheetIndex) => {
    ...
    if (
      !otherSheet ||
      sheet.sheet_id !== otherSheet.sheet_id ||
      sheet.rows.length !== otherSheet.rows.length
    ) {
      return false;
    }
    return sheet.rows.every((row, rowIndex) => {
      ...
      if (
        !otherRow ||
        row.row_index !== otherRow.row_index ||
...
function collectXlsxCellEdits(
  originalSheets: XlsxPreviewSheet[],
  editedSheets: XlsxPreviewSheet[],
) {
...
function DocumentChangelogPanel({
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-2">
      {entries.map((entry) => {
        const style = changeTypeStyles[entry.change_type] || {
          bg: 'bg-gray-100',
          text: 'text-gray-700',
        };
...
function extractPackageInfo(key: string): { packageId?: string; docType?: string } {
  ...
  try {
    ...
    const docType = filename.includes('_v') ? filename.split('_v')[0] : filename.split('.')[0];
...
export default function DocumentViewerPage({ params }: PageProps) {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      ...
      role: 'assistant',
      content:
        'I can help you review and edit this document. Ask me to add sections, modify content, or explain any part of the document.',
      timestamp: new Date(),
    },
  ]);
  ...
  const hasDocxPreviewChanges =
    docxPreviewBlocksEqual(editDocxPreviewBlocks, docxPreviewBlocks) === false;
  const hasXlsxPreviewChanges =
    xlsxPreviewSheetsEqual(editXlsxPreviewSheets, xlsxPreviewSheets) === false;
  ...
  const applyDocumentUpdate = useCallback(
    (doc: Partial<DocumentInfo> & { content: string }) => {
      if (doc.package_id) {
  ...
  const refreshGeneratedDocumentFromS3 = useCallback(
  ...
  useEffect(() => {
    ...
    try {
      const stored =
        ...
        sessionStorage.getItem(
          `doc-content-${encodeURIComponent(decodedId).replace(/\./g, '%2E')}`,
        );
      if (stored) {
    ...
    async function fetchDocument() {
      ...
      setActiveXlsxSheetId(null);
      setDocumentContent(
        '*Unable to load document content. The document may have been created in this session — try going back and clicking "Open Document" again.*',
      );
      setIsLoading(false);
    }
    ...
    const refreshBinaryDownloadUrl = async () => {
      try {
        ...
        const response = await fetch(
          `/api/documents/${encodeURIComponent(currentBinaryDownloadTarget)}?content=false`,
          {
            headers: token ? { Authorization: `Bearer ${token}` } : {},
          },
        );
...
${docSnippet}`;
  ...
  const buildDocumentAssistantPrompt = useCallback(
  ...
  const activeXlsxSheet =
    displayedXlsxSheets.find((sheet) => sheet.sheet_id === activeXlsxSheetId) ||
    displayedXlsxSheets[0] ||
    null;
  ...
  const updateDocxPreviewBlock = useCallback(
    (blockId: string, patch: Partial<DocxPreviewBlock>) => {
      setEditDocxPreviewBlocks((prev) =>
        prev.map((block) => (block.block_id === blockId ? { ...block, ...patch } : block)),
      );
    },
    [],
  );
        ...
        return {
          ...
          rows: sheet.rows.map((row) => ({
            ...
            cells: row.cells.map((cell) =>
              cell.cell_ref === cellRef ? { ...cell, value, display_value: value } : cell,
            ),
          })),
        };
      }),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const changedDocxPreviewBlocks =
      ...
      const result = (await res.json()) as DocumentInfo & {
        content?: string;
        message?: string;
        key?: string;
      };
      ...
      try {
        ...
        if (stored) {
          const docs = JSON.parse(stored) as Record<
            string,
            { id?: string; s3_key?: string; title?: string }
          >;
          const decodedId = decodeURIComponent(id);
          ...
          if (docs[normalizedId]) {
            delete docs[normalizedId];
            changed = true;
          }
  ...
  const handleDownload = async (format: 'docx' | 'pdf') => {
    ...
    try {
      ...
      const isDocx =
        sig.length >= 4 && sig[0] === 0x50 && sig[1] === 0x4b && sig[2] === 0x03 && sig[3] === 0x04;
      const isPdf =
        sig.length >= 4 && sig[0] === 0x25 && sig[1] === 0x50 && sig[2] === 0x44 && sig[3] === 0x46;
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-200 px-6 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {/* Export error */}
              {exportError && <span className="text-xs text-red-600 mr-2">{exportError}</span>}
              ...
              {/* Save error */}
              {saveError && <span className="text-xs text-red-600 ml-2">{saveError}</span>}
            </div>
          </div>
        </header>
        ...
        {showHydrationBanner && unfilledCount > 0 && (
          <div className="bg-amber-50 border-b border-amber-200 px-6 py-2 flex items-center justify-between">
            <p className="text-sm text-amber-800">
              This document has <strong>{unfilledCount}</strong> section
              {unfilledCount !== 1 ? 's' : ''} to be filled. The AI assistant will attempt to
              complete them.
            </p>
            <button
              ...
              <div className="flex-1 overflow-y-auto p-8">
                ...
                >
                  ...
                  <div className="space-y-6">
                    <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6 text-sm text-blue-950">
                      ...
                      <p className="mt-3 leading-6">
                        This Word document remains stored as a native `.docx` file. Browser edits
                        work on a structured preview and save back through `python-docx` so the
                        source file stays native.
                      </p>
                      <p className="mt-3 leading-6">
                        Use `Edit` for direct paragraph and checkbox updates here, or use the
                        assistant on the right for targeted DOCX revisions. Complex layout and
                        styling still remain in the native Word file.
                      </p>
                      {downloadUrl && (
                        <a
                          href={downloadUrl}
                            ...
                            <div className="flex items-center justify-between gap-4">
                              <div>
                                ...
                                <p className="mt-1 text-sm text-[#5d6b82]">
                                  Edit paragraphs and headings directly. Changes are saved back to
                                  the source document with formatting-preserving backend ops.
                                </p>
                              </div>
                          ...
                          <div className="space-y-5 p-5 md:p-8">
                            {editDocxPreviewBlocks.map((block) => (
                              <div
                                key={block.block_id}
                                ref={(element) => {
                                  docxBlockRefs.current[block.block_id] = element;
                                }}
                                ...
                                {block.kind === 'checkbox' ? (
                                  <div className="flex items-start gap-3">
                                    <input
                                      ...
                                      onChange={(e) =>
                                        updateDocxPreviewBlock(block.block_id, {
                  ...
                  <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6 text-sm text-blue-950">
                    ...
                    <p className="mt-3 leading-6">
                      This file is stored as a native `{fileType || documentType || 'binary'}`
                      document. Inline editing is intentionally disabled so EAGLE does not strip the
                      document formatting.
                    </p>
                    <p className="mt-3 leading-6">
                    ...
                    {contentType && (
                      <p className="mt-4 text-xs text-blue-800">Content type: {contentType}</p>
                    )}
                  </div>
                ) : isEditing ? (
                  <textarea
                      ...
                      >
                        <ReactMarkdown
                          components={{
                            ...
                            ul: ({ children }) => (
                              <ul className="list-disc ml-4 mb-1.5 space-y-0.5 text-xs">
                                {children}
                              </ul>
                            ),
                            ol: ({ children }) => (
                              <ol className="list-decimal ml-4 mb-1.5 space-y-0.5 text-xs">
                                {children}
                              </ol>
                            ),
                            strong: ({ children }) => (
