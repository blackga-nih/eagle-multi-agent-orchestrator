...
import { getDocumentStatusColor, formatDate } from '@/lib/format-helpers';
...
function getReadableDocumentType(type: string): string {
  ...
  if (type === 'pdf' || type === 'docx' || type === 'xlsx' || type === 'markdown' || type === 'txt')
    return type;
  return 'document';
}
...
function getStatusClass(status: string): string {
  ...
  if (supportedStatuses.has(status)) {
    return getDocumentStatusColor(
      status as 'not_started' | 'in_progress' | 'draft' | 'final' | 'approved',
    );
  }
  ...
  const statusTabs = useMemo(
    () => [
      { id: 'all', label: 'All Documents', badge: allDocuments.length },
      {
        id: 'not_started',
        label: 'Not Started',
        badge: allDocuments.filter((d) => d.status === 'not_started').length,
      },
      {
        id: 'in_progress',
        label: 'In Progress',
        badge: allDocuments.filter((d) => d.status === 'in_progress').length,
      },
      {
        id: 'draft',
        label: 'Draft',
        badge: allDocuments.filter((d) => d.status === 'draft').length,
      },
      {
        id: 'approved',
        label: 'Approved',
        badge: allDocuments.filter((d) => d.status === 'approved').length,
      },
    ],
    [allDocuments],
  );
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold ${
                        doc.document_type === 'sow'
                          ? 'bg-blue-500'
                          : doc.document_type === 'igce'
                            ? 'bg-green-500'
                            : doc.document_type === 'market_research'
                              ? 'bg-purple-500'
                              : doc.document_type === 'justification'
                                ? 'bg-amber-500'
                                : 'bg-gray-500'
                      }`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${getStatusClass(doc.status)}`}
                        >
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-500">
                        <span>
                          {documentTypeLabels[doc.document_type] || doc.document_type.toUpperCase()}
                        </span>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-xl">
                ...
                <div>
                  ...
                  <p className="mt-1">
                    <span
                      className={`inline-block text-xs font-bold uppercase px-2 py-1 rounded-full ${getStatusClass(selectedDocument.status)}`}
                    >
