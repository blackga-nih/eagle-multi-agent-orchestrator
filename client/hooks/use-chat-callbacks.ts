...
export function useChatCallbacks({
    ...
    const onMessage = useCallback((msg: { id: string; content: string; timestamp: Date; reasoning?: string; agent_id?: string; agent_name?: string }) => {
    ...
    const onComplete = useCallback((info?: StreamCompleteInfo) => {
        setAgentStatus(null);
    ...
    }, [currentSessionId, streamingMsgRef, streamingMsgIdRef, lastAssistantIdRef, firstUserMsgRef, titleGeneratedRef, setMessages, setToolCallsByMsg, setDocuments, setAgentStatus, setStreamingMsg, renameSession]);
    ...
    const onDocumentGenerated = useCallback((doc: DocumentInfo) => {
        ...
        if (currentSessionId) {
            const title = messages.find((m) => m.role === 'user')?.content.slice(0, 80) || 'Untitled Package';
    ...
    const onToolUse = useCallback((toolEvent: ToolUseEvent) => {
    ...
    const onAgentStatus = useCallback((status: string | null) => {
        setAgentStatus(status);
    }, [setAgentStatus]);

    const onStateUpdate = useCallback((metadata: Record<string, unknown>) => {
