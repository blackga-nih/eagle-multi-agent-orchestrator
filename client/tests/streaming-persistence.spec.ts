...
test.describe('Phase 1: Streaming Checkpoint', () => {
  ...
  test('Test 1: mid-stream refresh restores text with "Response interrupted" suffix', async ({
    page,
  }) => {
    ...
    await expect(page.locator('.typing-dot').first())
  ...
  test('Test 2: mid-stream refresh with tool calls and state changes — chips show correct status', async ({
    page,
  }) => {
    const sessionId = await getCurrentSessionId(page);
  ...
  test('Test 3: checkpoint cleared after normal stream completion (no phantom messages)', async ({
    page,
  }) => {
...
test.describe('Phase 2: Tool Call & State Change Persistence', () => {
  ...
  test('Test 7: tool call chips survive page refresh after completed response', async ({
    page,
  }) => {
  ...
  test('Test 8: tool call chips survive session switch and switch-back', async ({ page }) => {
    ...
    const sessionItems = page.locator(
      '[class*="space-y-0"] > div, [class*="session"], button:has-text("simplified")',
    );
  ...
  test('Test 11: session with no tool calls persists and restores without errors', async ({
    page,
  }) => {
  ...
  test('Test 17: interleaved tool chips + state change cards persist in correct order', async ({
    page,
  }) => {
    ...
    await expect(page.locator('[data-testid="tool-chip"]').first()).toBeVisible({ timeout: 5_000 });
    await expect(page.locator('[data-testid="state-change-card"]').first()).toBeVisible({
      timeout: 5_000,
    });

    // Check DOM order: tool chip should appear before state card
    const allInlineItems = page.locator(
      '[data-testid="tool-chip"], [data-testid="state-change-card"]',
    );
    ...
    expect(await page.locator('[data-testid="state-change-card"]').count()).toBeGreaterThanOrEqual(
      1,
    );
  });
...
test.describe('Edge Cases', () => {
  ...
  test('Test 13: empty stream with only complete event creates no phantom checkpoint', async ({
    page,
  }) => {
    const sse = buildSSE([
      {
  ...
  test('Test 14: error event during streaming preserves checkpoint for recovery', async ({
    page,
  }) => {
    const sse = buildSSE([
      {
  ...
  test('Test 19: localStorage quota pressure — large tool results are stripped', async ({
    page,
  }) => {
