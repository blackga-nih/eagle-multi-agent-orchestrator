...
test.describe('UC-02: Micro-Purchase Workflow', () => {
  test('micro-purchase request → simplified process with purchase request', async ({ page }) => {
    ...
    await expect(page.locator('.text-sm.text-gray-800.leading-relaxed').first()).toBeVisible({
      timeout: 60000,
    });
    ...
    await textarea.fill('The quote is for 3 centrifuge units at $4,600 each. Delivery in 2 weeks.');
    await expect(page.getByRole('button', { name: '➤' })).toBeEnabled();
    await page.getByRole('button', { name: '➤' }).click();

    // Wait for second assistant response
    await expect(page.locator('.text-sm.text-gray-800.leading-relaxed').nth(1)).toBeVisible({
      timeout: 60000,
    });
    await page.waitForTimeout(3000);
