...
const PROMPT =
  "I need to acquire a $2.5M bioinformatics data platform for NCI. It's a 3-year contract with IT services and cloud hosting. Help me determine the acquisition strategy, required documents, and applicable FAR provisions.";
...
test('UC: Complex Acquisition Package - $2.5M IT Services', async ({ page }) => {
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'uc-validate-complex-initial.png'),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'uc-validate-complex-before-send.png'),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'uc-validate-complex-streaming.png'),
    fullPage: true,
  });
  ...
  while (Date.now() - startTime < maxWait) {
    ...
    if (!found) {
      const bodyText = await page
        ...
        .catch(() => '');
      if (
        bodyText.toLowerCase().includes('acquisition') &&
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'uc-validate-complex-response.png'),
    fullPage: true,
  });
  ...
  const errorIndicators = await page
    .locator(
      '[role="alert"], .error, [class*="error"]:not([class*="console"]), [class*="Error"]:not([class*="Console"])',
    )
    .count();
  const bodyText = await page
    .locator('body')
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'uc-validate-complex-tools.png'),
    fullPage: true,
  });
  ...
  const activityToggle = page
    .locator(
      'button:has-text("Activity"), button:has-text("Logs"), button:has-text("Agent"), [aria-label*="activity" i]',
    )
    .first();
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'uc-validate-complex-activity.png'),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'uc-validate-complex-response-top.png'),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'uc-validate-complex-response-bottom.png'),
    fullPage: true,
  });
  ...
  const messagesPersistedUser =
    reloadLower.includes('bioinformatics') ||
    reloadLower.includes('2.5m') ||
    reloadLower.includes('data platform');
  const messagesPersistedAssistant =
    /FAR|acquisition/i.test(afterReloadBody) && afterReloadBody.length > 500;

  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'uc-validate-complex-reload.png'),
    fullPage: true,
  });
  ...
  console.log(
    `  Assistant response present:      ${contentChecks.assistantResponse ? 'PASS' : 'FAIL'}`,
  );
  console.log(
    `  Acquisition strategy mentioned:  ${contentChecks.acquisitionStrategy ? 'PASS' : 'FAIL'}`,
  );
  console.log(`  Dollar amount ($2.5M):           ${contentChecks.dollarAmount ? 'PASS' : 'FAIL'}`);
  console.log(
    `  TINA/cost or pricing data:       ${contentChecks.tinaOrCostPricing ? 'PASS' : 'FAIL'}`,
  );
  ...
  console.log(
    `  Small business/$750K:            ${contentChecks.smallBusiness ? 'PASS' : 'FAIL'}`,
  );
  console.log(`  No error messages:               ${contentChecks.noErrors ? 'PASS' : 'FAIL'}`);
  console.log(
    `  Input re-enabled:                ${contentChecks.inputReEnabled ? 'PASS' : 'FAIL'}`,
  );
  console.log('--- Tooling ---');
