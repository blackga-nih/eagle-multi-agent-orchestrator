...
test('UC: Simple Acquisition - Simplified Acquisition Threshold', async ({ page }) => {
  ...
  const existingMessages = await page
    .locator('[data-message-role="assistant"], [data-message-role="user"]')
    .count();
  if (existingMessages > 0) {
    console.log(`Found ${existingMessages} existing messages, looking for New Chat button`);
    const newChatBtn = page.locator(
      'button:has-text("New"), button[title*="new" i], button[aria-label*="new" i]',
    );
    if (
      await newChatBtn
        .first()
  ...
  if (!chatInput) {
    ...
    await page.screenshot({
      path: path.join(SCREENSHOTS_DIR, 'uc-validate-sa-ERROR.png'),
      fullPage: true,
    });
  ...
  const bodyAfterReload = await page
    .locator('body')
    .innerText()
    .catch(() => '');
  ...
  console.log(
    `  Tools detected:                   ${toolNames.length > 0 ? toolNames.join(', ') : 'none visible'}`,
  );
  console.log(`  Session persistence (user msg):   ${messagesPersistedUser ? 'PASS' : 'FAIL'}`);
  console.log(
    `  Session persistence (assistant):  ${messagesPersistedAssistant ? 'PASS' : 'FAIL'}`,
  );
