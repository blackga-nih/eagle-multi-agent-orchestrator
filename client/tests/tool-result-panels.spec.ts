...
const CHAT_INPUT = 'textarea[placeholder*="message"], textarea[placeholder*="Message"], input[placeholder*="message"], input[placeholder*="Message"]';
...
async function sendAndWaitForTools(page: Page, message: string, timeoutMs = 60_000) {
  ...
  await page.locator('[data-role="assistant"], .assistant-message, [class*="assistant"]').first().waitFor({ timeout: timeoutMs });
}
...
test.describe('Tool Result Panels', () => {
  ...
  test('document creation renders DocumentResultCard', async ({ page }) => {
    ...
    if (hasDocCard) {
      ...
      const card = page.locator('.border-t.border-\\[\\#E5E9F0\\]').filter({ has: page.locator('text=Open Document') }).first();
  ...
  test('reasoning (think) tool shows structured panel', async ({ page }) => {
    ...
    await sendAndWaitForTools(page, 'Compare the advantages of sole source vs full and open competition for a $500K IT contract');
    await waitForToolsComplete(page);

    // Look for think/reasoning tool card
    const thinkCard = page.locator('.rounded-lg.border.text-xs').filter({ hasText: 'Reasoning' }).first();
  ...
  test('S3 result panel shows file listing or content preview', async ({ page }) => {
    ...
    const s3Card = page.locator('.rounded-lg.border.text-xs').filter({ hasText: 'Document Storage' }).first();
  ...
  test('tool timing summary in agent logs', async ({ page }) => {
    ...
    const logsTab = page.locator('button').filter({ hasText: /logs|events/i }).first();
  ...
  test('subagent markdown report renders formatted', async ({ page }) => {
    test.setTimeout(120_000);
    await sendAndWaitForTools(page, 'Review the legal requirements for sole source justification over $250K', 90_000);
    await waitForToolsComplete(page);

    // Find a subagent card (Legal Analysis, Policy Analysis, etc.)
    const subagentLabels = ['Legal Analysis', 'Policy Analysis', 'Compliance Check', 'Policy Lookup'];
  ...
  test('web search panel renders answer and sources', async ({ page }) => {
    test.setTimeout(120_000);
    await sendAndWaitForTools(page, 'What are the latest OMB circular updates for federal acquisitions?', 90_000);
    await waitForToolsComplete(page);

    const webCard = page.locator('.rounded-lg.border.text-xs').filter({ hasText: 'Web Search' }).first();
  ...
  test('expandable chevron toggles on click', async ({ page }) => {
    ...
    const expandable = page.locator('.rounded-lg.border.text-xs').filter({ has: page.locator('text=▾') }).first();
    if (await expandable.count() > 0) {
      // Click to expand
      await expandable.locator('button').first().click();
